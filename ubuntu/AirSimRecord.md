...
- **np.max() np.maximum() np.argmax()**
  ...
  ./darknet detector train cfg/obj.data cfg/yolov3-tiny-obj.cfg backup/yolov3-tiny-obj_last.weights -map -dont_show
  
  ./darknet detector map cfg/obj.data cfg/yolov3-tiny-obj.cfg backup/yolov3-tiny-obj_last.weights
  ```
...
- **进度**
  ...
  -2000                5.6m
  # 6.26
  准备做数据集，系统boot空间出现问题，解决以后由于内核过高导致驱动出现问题，解决以后使用5.8.59，不能使用5.11版本，进入系统时不能直接选择Ubuntu，需要1>3的顺序进入，同时为了防止下次更新，关闭了自动更新以及锁定了当前的系统内核，同时重新生成了显卡驱动；
  更改creat_data文件，使用airsim导出的两个文件自动生成数据集，同时合并car1的之前5005数据集的1200张左右，合并代码在26晚补充完成，以后关于数据集的大部分工作可以通过脚本完成；771CAR3+1200CAR1训练50000代，模型准确度很低，需要检测一下样本label是否标记正确，再看数据增强的部分
  ...
  进行的是2步骤，需要下面需要进行步骤3来寻找模型准确率低的原因
  ```
  
  ```python
  # 6.27
  完成了test_label脚本，输入目标文件夹即可根据label检测图片标记是否正确，根据结果显示，标记往左偏了一部分，但角度等信息全部正确，手动修正即可
  小笔记：
  	np.arange(3,16) # 生成3到15的顺序数字
      str_line.strip('\n') # 一般用于读取文件时去除收尾的多余符号
      np.insert(arrays,index,values,axis) # 1待插入数组，2插入位置，3值，4行0和列1插入
      .astype(float) # 统一将array中str_类型数据转为float
  # 6.28
  ...
  311000-311273  car3绕圈半径13m
  312000-312035  car3倒退1
  312100-312127  car3前进1
  312200-312231  car3倒退2
  312300-312334  car3前进2
  输出模型：model_final_1415.pth
  ```
...
- **数据集**
  ...
  ```python
  #软链接：ln -s /home/wal/project/SMOKE/datasets/kitti2 datasets/kitti 源路径要完整
